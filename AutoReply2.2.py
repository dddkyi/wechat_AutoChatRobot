...
############################################
# 通用第一条消息模板
# FirstMsg = u'嘿嘿嘿，你的消息我已经收到了，但是主人现在木有在呢，小助手陪您聊天可以吗？[这是什么(W),聊天(C)，这很烦人(N)]'
FirstMsg = u'嘿嘿嘿，你的消息我已经收到了，但是主人现在木有在呢，小助手陪您聊天可以吗？[回复 "C" 由小助手和您聊天哦]'
# 星标好友额外消息模板
VIPMsgHead = u'小助手检测到你是:'
VIPMsgRear = u'，主人说你是对他特别重要的人，你稍等啦，或者也可以让我陪你聊天哈~'
# 对是否短信通知的回答及回复模板
...
ReplyWhat = '亲爱的朋友，这是微信代理小程序，如果您觉得这样的代理回复打扰到了您，可在任意时候回复 N ，即可永久停止我' \
            '与您之间的小助手回复代理。' \
...
def host_to_host(msg, msg_type):
    ...
    if msg_type == 'Text':
        ...
        # 别的文字消息则启动机器人回复
            ...
            robot_reply = utils.get_response(msg['Text'])
...
def other_to_host(session_id, msg, msg_type):
    ...
    # if SessionList[session_id]['HostIn']:
        # 发送给自己的消息类型
    if msg_type == 'Text':
        send_text = msg['Text']
    else:
